...
{
    // y值
    if (_imageView.height < self.bounds.size.height) {
        _imageView.top = floorf((self.bounds.size.height - _imageView.height) / 2.0);
    } else {
        _imageView.top = 0;
    }
}
...
//		[self zoomToRect:[self getRectWithScale:2 andCenter:touchPoint] animated:YES];
//        [self zoomScale:CGRectMake(touchPoint.x, touchPoint.y, 1, 1) animated:YES];
        [self zoomToRect:[self getRectWithScale:2.0 andCenter:touchPoint] animated:YES];

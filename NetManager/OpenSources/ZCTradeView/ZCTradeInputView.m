...
typedef enum {
    ...
    ZCTradeInputViewButtonTypeWithForgot = 30000,
...
/** 忘记密码按钮 */
@property (nonatomic, weak) UIButton *forgotBtn;
...
- (void)setupSubViews
{
    ...
    [self.forgotBtn addTarget:self action:@selector(btnClick:) forControlEvents:UIControlEventTouchUpInside];
    self.forgotBtn.tag = ZCTradeInputViewButtonTypeWithForgot;
    [self.forgotBtn setTitle:@"忘记密码?" forState:UIControlStateNormal];
    [self.forgotBtn setTitleColor:[UIColor colorWithHexString:@"#6699FF"] forState:UIControlStateNormal];


    /** 支付金额显示 */
    UILabel *moneyL = [[UILabel alloc] init];
    [self addSubview:moneyL];
    self.moneyLbl = moneyL;
    self.moneyLbl.textColor = [UIColor colorWithHexString:@"6699FF"];
    self.moneyLbl.textAlignment = NSTextAlignmentCenter;
    self.moneyLbl.font = [UIFont adaptFontOfSize:20.0];
...
- (void)layoutSubviews
{
    ...
    /** 忘记密码按钮 */
    CGSize forgotSize = [self.forgotBtn.titleLabel.text sizeWithAttributes:@{NSFontAttributeName:self.forgotBtn.titleLabel.font}];
    self.forgotBtn.y = self.cancleBtn.y - 35 * scaleW;
    self.forgotBtn.width = forgotSize.width;
    self.forgotBtn.height = 30 * scaleW;
    self.forgotBtn.x = CGRectGetMaxX(self.cancleBtn.frame) + ZCScreenWidth * 0.025 + self.cancleBtn.width - forgotSize.width;

    /** 支付金额显示 */
    self.moneyLbl.y = ZCScreenWidth * 0.60625 * 0.5 - ZCScreenWidth * 0.22625 * 0.5;
    self.moneyLbl.width = ZCScreenWidth - ZCScreenWidth * 0.0484375 * 2;
    self.moneyLbl.height = ZCScreenWidth * 0.20625 * 0.5;
    self.moneyLbl.x = ZCScreenWidth * 0.025;
}
...
- (void)btnClick:(UIButton *)btn
{
    ...
    } else if (btn.tag == ZCTradeInputViewButtonTypeWithForgot){
        
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ZCTradeInputViewForgotButtonClick" object:self];
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat y = ZCScreenWidth * 0.60625 * 0.5;
    ...
    CGFloat pointY = ZCScreenWidth * 0.34;
    CGFloat pointX;
